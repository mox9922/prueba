...
<form role="form" data-toggle="validator" method="POST" class="frm_data" id="frm_data"  autocomplete="off">
    ...
    <div class="modal-body">
        ...
        <div class="row mb-3">
            ...
            <div class="form-group col-md-4">
                <label>Monto</label>
                <input type="text" value="₡ <?=number_format($data->sv_valor)?>" readonly autofocus style="width:100%" class="form-control text-center imput_reset" autocomplete="off">
            </div>
        </div>
                    ...
                    foreach($comprobantes as $pg){
                        ...
                        if($this->user_data->u_perfil == 1){
                            echo "<tr> 
                                        <td>{$comprobante}</td>
                                        <td>{$fecha}</td>
                                        <td><button type='button' onclick='$(this).dell_data(\"{$id}\",\"url_delete_comprobante\")' class='btn btn-danger waves-effect waves-light'  data-toggle='tooltip' data-placement='top' title='Eliminar'><i class='fa fa-times text-white'></i></button></td>
                                  </tr>";
                        }else{
                            echo "<tr> 
                                        <td>{$comprobante}</td>
    ...
    <div class="modal-footer">
        ...
        <?=($this->user_data->u_perfil == 1) ? '<button type="submit" class="btn btn-success waves-effect waves-light">Guardar </button>' : '';?>
    </div>
